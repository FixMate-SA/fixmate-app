    ...
    try {
      ...
      if (response.ok && data.user) {
        ...
        if (loginResult.success) {
          ...
          if (loginResult.roleInfo?.role === 'client') {
            navigate('/client/dashboard');
          } else {
            setError('Account created but role mismatch detected. Please contact support.');
          }
        } else {
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">
            {currentStep === 1 ? 'Create Client Account' : 'Secure Your Account'}
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            {currentStep === 1 
              ? 'Join FixMate-SA to hire trusted fixers for your needs' 
