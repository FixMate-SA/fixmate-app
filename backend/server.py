from fastapi import FastAPI, APIRouter, Depends, HTTPException, File, UploadFile, Form, Request, Query
...
async def update_announcement(
    ...
    try:
        ...
        updatable_fields = ['title', 'content', 'target_audience', 'is_active', 'is_pinned', 
                           'priority', 'chat_enabled', 'admin_only_chat', 'expires_at']
        
        for field in updatable_fields:
            if field in request:
                setattr(announcement, field, request[field])
...
async def delete_announcement(
    ...
    """
    Delete an announcement and all associated chat messages.
    Admin only endpoint.
    """
    try:
        # Verify admin access
        ...
        # Get chat message count before deletion
        chat_count = db.query(AnnouncementChat).filter(AnnouncementChat.announcement_id == announcement_id).count()
        
        # Delete announcement (cascade will delete associated chat messages)
        db.delete(announcement)
        db.commit()
        
        return {
            "success": True,
            "message": f"Announcement deleted successfully along with {chat_count} chat messages"
        }
...
@api_router.delete("/announcements/{announcement_id}/chat/{message_id}")
async def delete_chat_message(
    ...
    """
    Delete a chat message (admin only or message owner).
    """
    try:
        # Get chat message
        chat_message = db.query(AnnouncementChat).filter(
            AnnouncementChat.id == message_id,
            AnnouncementChat.announcement_id == announcement_id
        ).first()
