...
def add_fixer(name, phone, skills):
    """Creates a new fixer in the database with correct formatting."""
    # --- THIS LOGIC IS NOW CORRECTED ---
    # It now correctly handles a 10-digit SA number
    ...
    existing_fixer = Fixer.query.filter_by(phone_number=whatsapp_phone).first()
    if existing_fixer:
...
def index():
    return "<h1>FixMate WhatsApp Bot is running.</h1>"
...
@login_required
def fixer_logout():
    logout_user(); flash('You have been logged out.', 'info'); return redirect(url_for('fixer_login'))

# --- NEW: Job Action Routes ---
...
def accept_job(job_id):
    ...
    if job.status == 'assigned':
        job.status = 'accepted'
        db.session.commit()
        client_message = f"Great news! Your Fixer, {job.assigned_fixer.full_name}, has accepted your job (#{job.id}) and is on their way."
