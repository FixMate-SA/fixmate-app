...
def transcribe_audio(media_url):
    ...
    try:
        ...
        if r.status_code == 200:
            # Twilio's ogg files can sometimes be tricky, let's try to be more robust
            try:
                ...
                print(f"Pydub conversion failed: {e}. Trying direct upload.")
                mime_type = 'audio/ogg'
                file_data = r.content

            print(f"Uploading audio to Gemini with MIME type: {mime_type}")
            
            gemini_file = genai.upload_file(file_data, mime_type=mime_type)
...
def whatsapp_webhook():
    ...
    if media_url and 'audio' in media_type:
        ...
        if transcribed_text:
            ...
            send_whatsapp_message(from_number, "Sorry, I couldn't understand that audio. Please try sending a text message instead.")
