...
class FixMateAPITester:
    ...
    # ======= ENHANCED JOB ASSIGNMENT WORKFLOW TESTING =======
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and 'analytics' in data:
                    ...
                    system_info = analytics.get('system_info', {})
                    
                    # Verify enhanced analytics features
                    analytics_features = []
                    if 'ai_services_status' in system_info:
                        ai_status = system_info['ai_services_status']
                        if ai_status.get('gemini_active'):
                            analytics_features.append("gemini_active")
                        if ai_status.get('openai_active'):
                            analytics_features.append("openai_active")
                    
                    generated_by = data.get('generated_by', 'unknown')
                    algorithm_version = system_info.get('algorithm_version', 'unknown')
                    
                    self.log_result("Admin Enhanced Matching Analytics", True, 
                                  f"Enhanced analytics retrieved: "
                                  f"Generated by: {generated_by}, "
                                  f"Algorithm: {algorithm_version}, "
                                  f"AI features: {', '.join(analytics_features)}")
                    return True
                else:
    ...
    def test_admin_matching_retrain(self):
        ...
        try:
            headers = {"Authorization": f"Bearer {self.test_data['admin_token']}"}
            # This endpoint doesn't take parameters, just POST
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    self.log_result("Admin Matching Retrain", True, 
                                  f"Algorithm retraining initiated: "
                    ...
                    message = data.get('message', 'Retraining failed')
                    completed_jobs = data.get('completed_jobs_count', 0)
                    self.log_result("Admin Matching Retrain", True, 
                                  f"Retraining response: {message} (Completed jobs: {completed_jobs})")
                    return True
                else:
